<aside class="main-sidebar sidebar-dark-warning elevation-4" style="background-image: url('{{Storage::url('public/imagenes/sistema/fondo-menu-1.jpg')}}'); background-size:auto 100%;background-repeat:no-repeat;background-attachment:fixed;">
    <div class="pantallaMenuLat" style="width: 100%;height: 100%;background-color: rgba(0, 0, 0, 0.6)">
        ...
        <a href="index3.html" class="brand-link" style="background-color: rgba(120, 200, 245, 0.8);">
            <img src="{{ asset('imagenes/sistema/'.session('logo_entidad')) }}" alt="Sistema Prueba" class="brand-image img-circle elevation-3" style="opacity: .8;max-width: 50px;">
            <div class="row">
                <div class="col-12">
                    <span class="brand-text font-weight-light" style="color: white;text-shadow: 1px 1px black;font-size: 0.9em;font-weight: bold;">{{ucwords(session('nombre_entidad'))}}</span>
                </div>
            </div>
        </a>
        <!-- Sidebar -->
               ...
               with font-awesome or any other icon font library -->
                    ...
                    @foreach ($menusComposer as $key => $item)
                        @if ($item["menu_id"] != 0)
                            @break
                        @endif
                        @include("admin.plantilla.menu-item", ["item" => $item])
                    @endforeach
                </ul>
            </nav>
            <!-- /.sidebar-menu -->
